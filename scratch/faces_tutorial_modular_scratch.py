...
def validate(netD, netG):
    ...
    for i, data in enumerate(dataloader, 0):
            ...
            outcome =  "Real"
            action = "Authentication: Granted"

            if output[0].item() < .5:
                outcome = "Fake"
                action = "Authentication: Denied"
            
            print("FREE-M.A.N. predicts sample is "+ outcome+ " with "  + str(abs(output[0].item() -.5)/.50) + " confidence. " + action )
    return 0
...
if __name__ == "__main__":
    ...
    if args.validate: 
        print("Validate Picture : " + args.validate +"\n" )
        validate(project[0][1],project[0][0])
